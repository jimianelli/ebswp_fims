...
#| fig-cap: "Fitted spawning biomass and expected recruitment trajectories from run0 and TVselex, with available ADMB 2024, SS3 base, and SAM poll24 derived outputs overlaid on comparable plotting scales. SAM SSB and age-1 recruitment are converted from kg/fish to thousands of tonnes and millions of fish, respectively."
make_diag_series <- function(estimates, model_name) {
  recruitment_values <- estimates |>
    filter(label == "expected_recruitment") |>
    mutate(year = map_year(year_i)) |>
    transmute(model = model_name, year, value = estimated, quantity = "Recruitment")

  ssb_values <- estimates |>
    filter(label == "spawning_biomass") |>
    mutate(year = map_year(year_i)) |>
    transmute(model = model_name, year, value = estimated, quantity = "SSB")
  ...
  derived$timeseries |>
    ...
    transmute(
      ...
      value = dplyr::case_when(
        model_name == "SAM poll24" & quantity == "SSB" ~ value / 1e6,
        model_name == "SAM poll24" & quantity == "Recruit_age1" ~ value / 1e6,
        TRUE ~ value
      ),
...
derived_diag_series <- bind_rows(
  ...
  read_derived_diag_series("ss3_base.rds", "SS3 base"),
  read_derived_diag_series("sam_poll24.rds", "SAM poll24")
)
...
ggplot(diag_series, aes(x = year, y = value, color = model)) +
  geom_line(linewidth = 0.8) +
...
#| fig-cap: "Five-peel retrospective trajectories for spawning biomass and recruitment when retrospective output is available. Failed peels are omitted from the plotted trajectories."
...
if (length(retro_inputs) > 0) {
  ...
  ggplot(retro, aes(x = year, y = value, color = factor(peel))) +
    geom_line(linewidth = 0.8) +
    facet_wrap(vars(run, quantity), scales = "free_y", ncol = 1) +
    scale_y_continuous(limits = c(0, NA)) +
  ...
  text(
    0.5, 0.5,
    "Five-peel retrospective output not found.\nCreate outputs/retro_5_peel_summary.rds or outputs/tvselex_retro_5_peel_summary.rds to populate this figure.",
    cex = 1
  )
}
```
