...
The full `run0` parameter listing is shown in @tbl-parameter-appendix. Standard deviations are reported when they are available in the fitted summary object; blank entries indicate that uncertainty output was not available for that parameter in the current saved fit.
