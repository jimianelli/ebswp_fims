...
- **Selectivity**: asymptotic logistic for fishery and BTS; double logistic for ATS and AVO
...
|---|---|---|
| `run0` | Initial FIMS run used for the current fitted results. | Fishery and BTS use time-invariant logistic selectivity; ATS and AVO use time-invariant double-logistic selectivity. |
| `TVselex` | Time-varying fishery selectivity run. | Fishery selectivity is changed to a double-logistic form and only the ascending inflection point is expanded over model years. |
...
The current fitted results are for `run0`. The fitted spawning biomass trajectory is shown in @fig-ssb. Fishery observed and predicted landings are compared in @fig-landings-fit, while survey and CPUE index fits are summarized in @fig-fits-by-fleet and residual patterns are shown in @fig-residuals. These index series are shown on their native arithmetic scales with separate facet scales because the magnitudes differ across surveys and CPUE, and the residuals are standardized by the input observation standard deviations on the arithmetic scale. Expected age-composition outputs for fishery, BTS, and ATS are shown in @fig-agecomp-fishery, @fig-agecomp-bts, and @fig-agecomp-ats, respectively, while selectivity is shown in @fig-selectivity-curves. The time-invariant selectivity assumptions used to initialize each gear are shown for selected years in @fig-selectivity-assumptions. A diagnostics overview, including model status and the fitted SSB and recruitment trajectories, is shown in @tbl-model-status and @fig-ssb-recruitment. Likelihood diagnostics and key parameter summaries are reported in @tbl-diagnostics, @tbl-recruitment-parameters, and @tbl-q.
...
#| fig-cap: "Illustrative selectivity assumptions by gear for selected years. Fishery and BTS are initialized with asymptotic logistic selectivity, while ATS and AVO use double-logistic assumptions. In the current implementation these assumptions are time-invariant, so the curves are identical across years."
...
This section summarizes basic model-status diagnostics for the current fit and provides a place to evaluate a 5-peel retrospective case when retrospective output is available.
...
#| fig-cap: "Fitted spawning biomass and expected recruitment trajectories from the current model fit."
recruitment <- est |>
  filter(label == "expected_recruitment") |>
  mutate(year = map_year(year_i)) |>
  transmute(year, value = estimated, quantity = "Recruitment")

ssb_diag <- est |>
  filter(label == "spawning_biomass") |>
  mutate(year = map_year(year_i)) |>
  transmute(year, value = estimated, quantity = "SSB")

diag_series <- bind_rows(ssb_diag, recruitment)

ggplot(diag_series, aes(x = year, y = value)) +
  geom_line(color = "#2c7fb8", linewidth = 0.8) +
  facet_wrap(~quantity, scales = "free_y", ncol = 1) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = "Year", y = NULL, title = "Spawning Biomass and Recruitment")
```

```{r}
#| label: fig-retro-5-peel
#| fig-cap: "Five-peel retrospective trajectories for spawning biomass when retrospective output is available."
retro_path <- file.path(project_root, "outputs", "retro_5_peel_summary.rds")

if (file.exists(retro_path)) {
  retro <- readRDS(retro_path)

  ggplot(retro, aes(x = year, y = value, color = factor(peel))) +
    geom_line(linewidth = 0.8) +
    facet_wrap(~quantity, scales = "free_y", ncol = 1) +
    scale_y_continuous(limits = c(0, NA)) +
    labs(x = "Year", y = NULL, color = "Peel", title = "Five-Peel Retrospective Diagnostics")
} else {
  plot.new()
  text(
    0.5, 0.5,
    "Five-peel retrospective output not found.\nCreate outputs/retro_5_peel_summary.rds to populate this figure.",
    cex = 1
  )
}
```
...
The reduced `TVselex` formulation completed, but the maximum gradient remains large, so these results should be treated as a diagnostic attempt rather than an accepted comparison model. The fit produced a total NLL of `r tvselex_status$value[tvselex_status$metric == "Total NLL"]` and terminal spawning biomass of `r tvselex_status$value[tvselex_status$metric == "Terminal SB"]`, but these quantities should not be interpreted until the convergence problem is resolved. The next iteration should add structure to the annual ascending-inflection estimates, such as time blocks, smoothing, bounds, or penalties.

```{r}
#| label: tbl-tvselex-status
#| tbl-cap: "Status summary for the attempted TVselex model run."
tvselex_status |>
  gt_report() |>
  tab_header(title = "TVselex Run Status")
```
...
#| tbl-cap: "Estimated initial numbers-at-age in thousands."
naa_tbl <- est |>
...
initial_naa_tbl |>
  ...
  tab_header(title = "Initial Numbers-at-Age (Thousands)")
```

```{r}
#| label: tbl-terminal-naa
#| tbl-cap: "Estimated terminal numbers-at-age in thousands."
...
terminal_naa_tbl |>
  gt_report() |>
  fmt_number(columns = value, decimals = 3) |>
  tab_header(title = "Terminal Numbers-at-Age (Thousands)")
```
...
The full parameter listing is shown in @tbl-parameter-appendix. Standard deviations are reported when they are available in the fitted summary object; blank entries indicate that uncertainty output was not available for that parameter in the current saved fit.

```{r}
#| label: tbl-parameter-appendix
#| tbl-cap: "Appendix table of fitted parameter estimates and standard deviations, where available."
...
parameter_appendix |>
  ...
  tab_header(title = "Appendix: Parameter Estimates")
```
